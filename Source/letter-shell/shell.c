...
#else
static void setVariable(uint8_t argc, char **argv)
{
    SHELL_TypeDef *shell = shellGetCurrent();
    if(argc < 3)
    {
        shellDisplay(shell, "Please input:setVar [var] [value]\r\n");
    }
    else
    {
        shellSetVariable(argv[1], atoi(argv[2]));
    }
}
SHELL_EXPORT_CMD_EX(setVar, setVariable, set var, setVar $[var] [value]);
#endif
